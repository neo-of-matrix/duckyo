 ...
 * @param cwd
 * @param pkg
 * @returns eslint-config-duckyo/index
 * @returns eslint-config-duckyo/react
 * @returns eslint-config-duckyo/typescript/index
 * @returns eslint-config-duckyo/typescript/react
 */
export function getESLintConfigType(cwd: string, pkg: PKG): string {
  ...
  return (
    'eslint-config-duckyo/' + `${language}/${dsl}`.replace(/\/$/, '/index').replace(/^\//, '')
  );
}
