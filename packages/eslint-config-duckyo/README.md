...
```shell
npm i -D eslint-config-duckyo @babel/core @babel/eslint-parser eslint-plugin-import
```
...
```shell
npm i -D eslint-config-duckyo @babel/core @babel/eslint-parser eslint-plugin-import eslint-plugin-react eslint-plugin-react-hooks
```
...
```json
{
  "extends": ["eslint-config-duckyo/react"]
}
```
...
```json
{
  "extends": ["eslint-config-duckyo/react", "eslint-config-duckyo/jsx-a11y"]
}
```

## JavaScript + Vue 项目 - eslint-config-duckyo/vue
...
## JavaScript (Node.js) 项目 - eslint-config-duckyo/node
...
```shell
npm i -D eslint-config-duckyo @typescript-eslint/parser @typescript-eslint/eslint-plugin eslint-plugin-import eslint-import-resolver-typescript
```
...
```json
{
  "extends": ["eslint-config-duckyo/typescript"]
}
```
...
```
npm i -D eslint-config-duckyo @typescript-eslint/parser @typescript-eslint/eslint-plugin eslint-plugin-import eslint-import-resolver-typescript eslint-plugin-react eslint-plugin-react-hooks
```
...
```json
{
  "extends": ["eslint-config-duckyo/typescript/react"]
}
```
...
```json
{
  "extends": [
    ...
    "eslint-config-duckyo/jsx-a11y"
  ]
}
```

## TypeScript + Vue 项目 - eslint-config-duckyo/typescript/vue
...
## TypeScript (Node.js) 项目 - eslint-config-duckyo/typescript/node
...
```json
{
  "extends": ["eslint-config-duckyo/typescript/react", "prettier"]
}
```
...
- 了解如何在继承本包的基础上对项目 ESLint 进行个性化配置，可参考官网的 [Configuring ESLint](https://eslint.org/docs/user-guide/configuring)。下面简介下 ESLint 配置中的几个常用字段：
  - `extends`: 继承一组规则集。`"extends": "eslint-config-duckyo",` 表示继承本包定义的规则配置。
